...
import { BookingStatus, PLATFORM_FEE_MAP, ALLOWED_PASS_DAYS } from "./booking.constants";
...
export async function createBookingTransaction(payload: any) {
  return db.transaction(async (tx) => {
    ...
    while (remainingDays > 0) {
      const isHoliday = await BookingRepository.isHoliday(tx, facilityId, cursor);

      if (!isHoliday) {
        const slot = await BookingRepository.lockFacilitySlot(
          tx, facilityId, cursor, slotType
        );
